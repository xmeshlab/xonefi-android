...
import AsyncStorage from '@react-native-async-storage/async-storage';
import {
    Callback, CallbackWithResult, MultiCallback, MultiGetCallback
...
export const createTypeSafeStorage = <T extends {}>() => {
    ...
    return {
        async isEmpty() {
            const tasks = await Promise.all(Object.keys(onefiSimpleData)
                .map(key => AsyncStorage.getItem(key)
                    .then(res => ({key, value: JSON.parse(res)}))))
        ...
        async init(data: Partial<T>) {
            await Promise.all(Object.keys(data)
                .map(key =>  AsyncStorage.setItem(key, JSON.stringify(data[key]))));
        },
        ...
        async getAllKeys(callback?: CallbackWithResult<readonly string[]> | undefined): Promise<readonly Key[]> {
            return (await AsyncStorage.getAllKeys(callback)) as Key[];
        },
        mergeItem<K extends Key>(key: K, value: T[K], callback?: Callback | undefined): Promise<void> {
        ...
        async multiGet<K extends Key>(keys: readonly K[], callback?: MultiGetCallback): Promise<[K, T[Key]][]> {
            return (await AsyncStorage.multiGet(keys as unknown as string[], callback))
                .map(([key, value]) => {
                    return [key as unknown as K, JSON.parse(value) as T[K]]
                });
        },
        multiMerge<K extends Key>(keyValuePairs: [K, T[K]][], callback?: MultiCallback): Promise<void> {
            const newKeyValuePairs: [string, string][] = keyValuePairs.map(([key, value]) => {
        ...
        multiSet<K extends Key>(keyValuePairs: [K, T[K]][], callback?: MultiCallback): Promise<void> {
            const newKeyValuePairs: [string, string][] = keyValuePairs.map(([key, value]) => {
                return [key as string, JSON.stringify(value)];
            });
        ...
        setItem<K extends Key>(key: K, value: T[K], callback?: Callback): Promise<void> {
            const val = JSON.stringify(value);
        ...
        async getItem<K extends Key>(key: K, callback?): Promise<T[K] | null> {
            ...
            const text = await AsyncStorage.getItem(key as unknown as string, callback);
