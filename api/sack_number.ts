 ...
 */
export function get_sack_number() {
    // const config  = require("./config");
    // let config_json = config.read_default_config();
    // return config_json.client_session.sack_number;
    return  OneFiStorage.getItem('client_session').then(r => r.sack_number);
}
 ...
 */
export async function set_sack_number(number) {
    const clientSession = await OneFiStorage.getItem('client_session');
    clientSession.sack_number = number;
    OneFiStorage.setItem('client_session', clientSession);
    return true;
}
