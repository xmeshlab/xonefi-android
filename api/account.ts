...
export async function generatedAccount(seed:string, password:string) {
    ...
    const timed_seed = Date.now() + uuid.v4().toString() + deviceName + totalMemory + usedMemory;
    const account = web3.eth.accounts.create(timed_seed);
    res.address = account.address;
    res.privateKey = account.privateKey;
    res.encryptedPrivateKey = encrypt_aes256ctr_base64(account.privateKey, password);

    return res;
}

export async function saveAccount (address: string, privateKey: string, encryptedPrivateKey: string) {
