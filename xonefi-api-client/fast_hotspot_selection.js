...
function fast_hotspot_selection_ng2(results, config_json) {
  ...
  let private_prefixes =
    private_provider_prefixes.get_private_provider_prefixes(config_json);
  ...
  for (let i = 0; i < results.ssids.length; i++) {
    ...
    if (dhpt.access_method === "restricted") {
      ...
      if (!private_prefixes.includes(pref)) {
        // If the prefix does not match any of the trusted providers, ...
...
module.exports = {
  ...
  fast_hotspot_selection_ng,
  fast_hotspot_selection_ng2,
};
