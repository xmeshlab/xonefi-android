...
function client_worker(config_json, user_password, private_key, callback) {
  ...
  console.log(
    `@DEB5 config_json.client_session.initiated_sack_number: ${config_json.client_session.initiated_sack_number}`
  );
  console.log(
    `@DEB5 config_json.client_session.sack_number: ${config_json.client_session.sack_number}`
  );
  ...
  ssid.ssid_scan(config_json.wlan_interface, (networks) => {
    ...
    if (config_json.client_session.status === session_status.status.ACTIVE) {
      ...
      if (config_json.client_session.pft || config_json.client_session.pfd) {
        let best_candidate_ssid =
          results.ssids[fhs.fast_hotspot_selection_ng(results)];
        console.log(`BEST CANDIDATE SSID:             ${best_candidate_ssid}`);
        console.log(
          `config_json.client_session.ssid: ${config_json.client_session.ssid}`
        );

        if (
          handover_helper.handover_match(
