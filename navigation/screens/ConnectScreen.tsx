...
import {
    View, Text, ImageBackground, FlatList, TouchableOpacity, StyleSheet, ActivityIndicator, Platform
} from 'react-native'
//for wifi
...
import { RouteComponent, WiFiLevel, WifiWithSignalLevel } from '../../types/global';
...
const ConnectScreen:RouteComponent<'Connect'> = () => {
    ...
    const flatListRenderItem = useCallback(({item}) => (<TouchableOpacity
            onPress={() => navigation.navigate('PayAndConnect', {
                BSSID: item.BSSID, SSID: item.SSID
            })}
        >
            <WifiItem
                frequency={item.frequency}
                signalLevel={item.signalLevel}
                key={item.BSSID} SSID={item.SSID} price={0.8}/>
        </TouchableOpacity>), [])
    ...
    return (<View style={{flex: 1}}>
            <View style={style.tabBar}>
                <View style={style.tabBarInner}>
                    {tabBtnList.map(tab => {
                        const tabStyle = [style.tabBtn, currentActive === tab ? style.tabBtnActive : null];
            ...
            <View style={style.mgnTop}>
                <FlatList<WifiEntry> data={wifiList}
                          onRefresh={getWifiList}
                          refreshing={isLoading}
                          ListEmptyComponent={<PrimaryBtn style={style.connectBtn} onPress={getWifiList}>
...
const WifiItem = ({SSID, price, frequency, signalLevel}: Partial<WifiWithSignalLevel> & { price: number }) => {
    return <View style={style.wifiItem}>

        <View style={[style.wifiItemText, globalStyle.row, globalStyle.verticalCenter, {marginRight:15}]}>
            <WifiLevelIcon signalLevel={signalLevel} />

            <LockICon style={{marginLeft:15, marginTop:5}}/>

        </View>
        <Text style={style.wifiItemText}>
            {SSID}
        </Text>
        <Text style={style.wifiItemText}>
            {price} OFI/GB
...
const style = StyleSheet.create({
    mgnTop: {marginTop: 80},
    tabBar: {
        marginTop: 18, height: 51,
        backgroundColor: 'rgba(40, 40, 40, 0.48)', //
        position: 'absolute', left: 15, right: 15, // alignItems: 'center',
        justifyContent: 'center', borderRadius: 10,

    }, tabBarInner: {
        paddingLeft: 12, paddingRight: 12, flexDirection: 'row', justifyContent: 'space-between'
    }, tabBtn: {
        color: '#fff',
        width: 97,
        height: 33,
