...
const InputBar = ({IconComponent, TokenName}) =>{
    return (<View className="flex flex-row mx-4 mb-1 mt-2 justify-between rounded-3xl border-slate-800 bg-slate-800 pl-3 pr-3 py-1 items-center" >
                    <TextInput className="w-4/6" selectionColor="#FFF" placeholderTextColor="#FFF" placeholder="" keyboardType='numeric'/>
  ...
  const [currentActive, setCurrentActive] = useState('Buy');
    ...
    return (
        <ImageBackground source={backgroundImage} resizeMode="cover" className="flex-1 flex-col">
            <View style={style.tabBar}>
                <View style={style.tabBarInner}>
                    {tabBtnList.map(tab => {
                        ...
                        const textStyle = [style.tabBtnText, currentActive === tab ? style.tabBtnActiveText : null];
            ...
            <View className="mt-4">
              <ButtonBackgroundSVG className="mx-4" ButtonText={"Complete Purchase"}/>
            </View>
...
const style = StyleSheet.create({
  ...
  tabBar: {
      ...
      paddingLeft: 12, paddingRight: 12, flexDirection: 'row', justifyContent: 'space-between', marginLeft: 1, marginRight:1
  }, tabBtn: {
      color: '#fff',
      width: 97,
      height: 33,
