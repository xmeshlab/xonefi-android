...
export function BlurModal({onContentPress, children, ...modalProps}: BlurModalProps) {
    const wrapperOnContentPress = useCallback((event: GestureResponderEvent) => {
        event.stopPropagation();
        onContentPress?.(event);
    }, [onContentPress])
    const style = {
        position: 'absolute', height: 98, width: 300, paddingLeft: 48, paddingRight: 48, backgroundColor: '#1E2732',
    };
    return <Modal animationIn={'fadeIn'}  {...modalProps}>
        <BlurView blurType={'light'} style={modalStyle.container}>

            <TouchableOpacity activeOpacity={1} onPress={modalProps.onBackdropPress}
                              style={modalStyle.backdrop}>
                <TouchableOpacity activeOpacity={1} onPress={wrapperOnContentPress} style={modalStyle.modalCard}>
